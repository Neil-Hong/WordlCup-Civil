...
function TeamStateCard({
  team, teamState, winRate, side,
}: {
  team: { id: string; name: string; flag: string; color: string };
  teamState: { attack: number; defense: number; midfield: number; momentum: number; supporter: number };
  winRate: number;
  side: "A" | "B";
}) {
  return (
    <div className="rounded-md border border-white/10 bg-white/[0.025] p-3">
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-[10px] text-gray-500 ml-auto">Win: {winRate}%</span>
      </div>
      ...
      <div className="mb-2 h-1.5 overflow-hidden rounded-sm bg-white/[0.06]">
        <div
          ...
          style={{ width: `${winRate}%`, backgroundColor: team.color, opacity: 0.7 }}
        />
      </div>
...
export function AgentFeed({ matchId, homeTeam, awayTeam, initialAgent }: AgentFeedProps) {
  const [agent, setAgent] = useState<MatchAgentState | null>(initialAgent ?? null);
  ...
  if (!agent) {
    return (
      <div className="glass-panel p-8 text-center text-gray-500">
        <p className="text-sm">Match Agent will activate once the match starts.</p>
        <p className="text-xs mt-1 text-gray-600">
          The autonomous AI Match Agent analyzes both teams and updates predictions in real-time.
        </p>
      </div>
    );
  }
    ...
    <div className="glass-panel p-5">
      ...
      <div className="mb-4 flex items-center gap-3">
        <div className="flex items-center gap-1">
          <span className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: agent.source === "somnia-llm" ? "#c49b38" : "#8a96a8" }} />
          <span className="section-label text-[10px]">
            {agent.source === "somnia-llm" ? "Somnia LLM Active" : "Agent Processing"}
          </span>
        </div>
        <span className="text-[10px] text-gray-500">
          Win Prediction: <span className="text-white font-bold">{homeTeam.name} {agent.teamAWinRate}%</span>
          {" - "}
          <span className="text-white font-bold">{awayTeam.name} {agent.teamBWinRate}%</span>
        </span>
      ...
      <div className="mb-4 grid grid-cols-2 gap-4">
        <TeamStateCard team={homeTeam} teamState={agent.teamA} winRate={agent.teamAWinRate} side="A" />
        <TeamStateCard team={awayTeam} teamState={agent.teamB} winRate={agent.teamBWinRate} side="B" />
      </div>
      ...
      {agent.predictedScore && agent.predictedScore !== "?-?" && (
        <div className="text-center mb-4">
          ...
          <span className="font-mono text-sm font-semibold text-white">{agent.predictedScore}</span>
        </div>
      )}
        ...
        <div ref={scrollRef} className="space-y-2 max-h-44 overflow-y-auto pr-1">
          {commentaries.map((msg, i) => {
            ...
            return (
              ...
              >
                <span className={`mr-1 font-mono text-[9px] font-semibold ${isSomnia ? "text-gold" : "text-gray-500"}`}>
          ...
          {commentaries.length === 0 && (
            <p className="text-[10px] text-gray-600 italic">Waiting for Match Agent input...</p>
          )}
        </div>
      </div>
    </div>
